...
const getLights = async () => {
  fetch("https://codember.dev/colors.txt")
    ...
    .then((data) => {
      const colors = JSON.parse(data.replaceAll("'", '"'));
      ...
      colors.forEach((color) => {
        if (firstColor === "" || secondColor === "") {
          if (firstColor === "") {
            ...
            if (countZebra >= longestZebra) {
              longestZebra = countZebra;
              lastColor = color;
            }
          } else if (secondColor === "" && firstColor !== color) {
            secondColor = color;
            countZebra++;
      ...
      console.log(`${longestZebra}@${lastColor}`);
    })
    .catch((error) => {
      console.log(error);
    });
